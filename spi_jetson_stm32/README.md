...
响应为 `60 STATUS RESULT[N] CRC_LO CRC_HI 0A`，逻辑长度为 5～256 字节。回调填写返回数据和实际长度，最后填执行状态，后台随后计算 CRC。第二次 SPI 事务提供 256 字节时钟，逻辑帧后的 `0xFF` 是填充。单次写操作失败时执行状态可能不确定，程序不自动重试。
