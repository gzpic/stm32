...
2026-09-12 板载传感器命令：新增 `F0/02` 和 `F0/03`。`F0/02` 使用 ADC1 通道 16 读取 STM32F407 内部温度传感器，返回 2 字节小端有符号 `0.01°C`；Jetson 实测首次数值 `9D 0B`，即 `29.73°C`，后续五次为约 `31.98~32.30°C`。`F0/03` 使用 PF7/ADC3 通道 5 读取板载 LS1 光敏传感器，返回 1 字节 `0~100` 相对光照强度，六次实测均为 `18`。Keil 工程和 Jetson `make test`、`make all` 均通过，新增命令连续读取成功。内部温度转换沿用原实验20的典型参数，表示 MCU 结温近似值，未经单板校准；光照值不是 lux。详细帧和结果见 `docs/I2C_TEST_RECORD.md`。
