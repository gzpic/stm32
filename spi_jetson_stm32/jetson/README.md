...
本目录实现 Linux `spidev` 主机程序。程序根据命令行参数构造写命令帧，完成第一次 SPI 写事务；等待 STM32 后台处理后，再执行第二次 SPI 事务读取固定长度响应并校验。帧编解码复用 `../common/protocol.c`。
...
4. 再发送 36 个 `0xFF` 占位字节以产生时钟，同时从 MISO 读取响应。
5. 校验 `60 STATUS RESULT[31] CRC_LO CRC_HI 0A`，输出状态和 31 字节结果。
...
联调时建议先使用 100 kHz 和默认回显命令，通过逻辑分析仪确认两个独立 CS 低窗口、写帧 `0x30`、返回帧 `0x60`、36 字节读长度及至少 10 ms 的高电平间隔，再逐步提高频率或增加业务命令。
